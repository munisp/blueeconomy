...
## Current weighted completion score: **51.00%**

The current calculation is `(15×65 + 15×50 + 15×50 + 15×45 + 10×50 + 10×35 + 10×50 + 10×60) ÷ 100 = 51.00%`. The previous 50.25% score remains the historical pre-S2-Kafka baseline; 48.25% remains the historical pre-S4-registry baseline; 47.50% remains the historical pre-S2-casework baseline, 46.75% remains the pre-S2 geofence baseline, 44.75% remains the pre-Mojaloop/JTI baseline, 44.00% remains the post-genealogy baseline, 43.50% remains the prior pre-genealogy baseline, and 34.00% remains the prior major baseline. The new 0.75-point increase credits only the authentic S2 Kafka broker delivery test. The S3 orchestration executable is not credited as TigerBeetle integration until a live local TigerBeetle cluster or approved Mojaloop participant side-effect test is recorded. It is **not** a production-readiness score, a partner-integration score, or an operational-acceptance score.
