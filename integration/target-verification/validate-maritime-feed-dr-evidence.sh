...
manifest_values=$(jq -er --arg suite "$suite" '
  select(.schema_version == "blueeconomy.target-test-manifest.v2") |
  select(.suite == $suite) |
  ...
  [.target_environment,.run_id,.runbook_sha256,.artifact_root_id] | @tsv
' "$manifest") || usage_error 'manifest lacks approved v2 authorisation, runbook-digest, or artifact-root bindings'
IFS=$'\t' read -r target_environment run_id runbook_sha256 artifact_root_id <<< "$manifest_values"
...
jq -e \
  ...
  --arg artifact_root_id "$artifact_root_id" '
    .schema_version == "blueeconomy.target-test-summary.v2" and
    .suite == $suite and
...
jq -n \
  --arg schema_version 'blueeconomy.maritime-dr-evidence-validation.v2' \
  --arg suite "$suite" \
  ...
  '{schema_version:$schema_version,suite:$suite,target_environment:$target_environment,run_id:$run_id,runbook_sha256:$runbook_sha256,artifact_root_id:$artifact_root_id,validated_at:$validated_at,status:"complete",validated_cases:$ARGS.positional}' \
